...
The original methodological contribution of this analysis, for this extract, is a framework for aggregation-identifiability and calibrated refusal. It is not a new estimator, and it does not claim priority over existing methods for aggregated outcomes [15,16]. At month grain, exploratory ecological associations of first-event counts with monthly thermal summaries are recoverable. Daily lag structure was not recovered here: a pre-specified simulation calibration failed, so no real daily coefficient is reported [15,16]. Heatwave excess-death mortality is a different estimand class. AMI or principal-diagnosis effects, stroke coefficients, and cohort incidence are refused because the required fields were not delivered. Four displays show where the recoverable information comes from: which months carry the exposure contrast, whether the outcome decline is compatible with risk-set depletion, how far each interval moves across standard-error constructions, and which specifications each estimate depends on. When a named check fails, that object is not estimated. The twelve-contrast family is reported in full. Every *q*-value exceeds 0.19. This analysis does not meet the bar for a protected primary claim.
