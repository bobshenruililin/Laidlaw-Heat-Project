...
run_one <- function(script, env = character()) {
  message("\n========== RUNNING ", script, " ==========\n")
  rc <- system2("Rscript", script, env = c(paste0("PATHWAY_MODE=", mode), env))
...
for (outcome in outcomes) {
  ...
  # Copy forest with outcome-specific name
  src <- file.path(root, "outputs", "figures", "pathway", "pathway_panel_forest.png")
  if (file.exists(src)) {
    file.copy(src, file.path(root, "outputs", "figures", "pathway", paste0(outcome, "_pathway_panel_forest.png")), overwrite = TRUE)
  }
}
