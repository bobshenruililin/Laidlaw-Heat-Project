  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Jasmine source confirmed; extend definitions to 2023
      \end{itemize}
      \vspace{0.55em}
...
\begin{frame}{What each person still unlocks}
  ...
  \vspace{0.1em}
  \footnotesize
  {\color{LaidlawTeal}\textbf{Hogan:}} lock weather / hot-month / cold-month definitions, including Roro's four as named siblings.\\
  {\color{LaidlawTeal}\textbf{Roro:}} first GOPC mention $\to$ true event month; transfer; revised PDF vs medRxiv v1.\\
  {\color{LaidlawTeal}\textbf{Dr Bishai:}} thank you for identifying Jasmine as Jingwen Liu et al.\ (2020).\\
...
\begin{frame}{Literature anchors (selected)}
  ...
  \begin{itemize}
    ...
    \item \textbf{Jingwen Liu et al.\ 2020} --- confirmed Jasmine paper; cold $\gg$ heat mortality AF
    \item \textbf{Zhenyuan Liu et al.\ 2026} --- team multi-definition excess-heat mortality baseline
...
%----------------------------------------------------------------------
% Jasmine signal + hot/cold month spin-out (next week)
\input{_jasmine_hot_cold_frames.tex}
...
\begin{frame}[plain]
  ...
  \begin{itemize}
    \item \textbf{Hogan:} shall we lock the heatwave / hot-month definitions together
      (including your count$\to$upper-tail idea, the cold starters, and Roro's four)?
    \item \textbf{Roro:} does \texttt{revised manuscript\_clean.pdf} differ from medRxiv v1,
      and can we lock first-GOPC$\to$true-stroke timing plus aggregate-transfer timing?
  \end{itemize}
