...
**Paste source:** `Heat_CVD_Manuscript_20260812_collab_draft.docx` (rebuilt after the honesty pass below).  
...
2. **Abstract.** Replace the yellow “work in progress” with the draft Abstract. Keep exploratory language (all twelve *q* > 0.19).
3. **Introduction.** Replace the 28 July text. This answers his comment threads: AMI removed as *our* endpoint; housing/behaviour/medication omitted; 2013–2023 EPD change documented; thesis combined into one closing sentence; Yang CY kept. Reply on each comment using `hogan_comment_responses.md`.
...
7. **Results.** Replace “work in progress” with Tables 1–3 and the surrounding prose. Optional figures from `outputs/release_chd_hf/figures/` if he wants them in the shared file.
8. **Discussion / Conclusion / limitations.** Replace “work in progress.” Keep the refusal of a protected primary claim.
9. **References.** Keep his 1–8 (C&SD repaired as 8). Add 9–20 from the draft. Do not drop Yang CY.

## Honesty taken from later drafts (approved for *this* file only)

These were proposed for the Laidlaw report. They belong here instead.

| Taken | Why |
|---|---|
| NW3 CHD hot nights as 1.0003–1.0439 (unrounded 1.000253–1.043860) | Display honesty; Model/HC1 still include 1 |
| Leading-contrast ACF 0.508 / 0.146 | Why Newey–West is shown |
| Cold days 141/145 in DJF; remaining variation is between-year winter | Identification, not a new model |
