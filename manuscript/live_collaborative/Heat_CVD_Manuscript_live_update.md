...
*Author order after the first and last positions remains to be confirmed with the supervising investigator.*
...
The dependent variable is the monthly count of first hospitalisations after a first diagnosis of CHD, and separately of HF, among people with type 2 diabetes and/or hypertension in Hong Kong. The outcome co-investigator built and released these monthly counts. The cohort is people diagnosed with type 2 diabetes and/or hypertension during 2013–2023. For each person, the event is the first recorded hospitalisation after that person’s first CHD diagnosis or first HF diagnosis. A clinical collaborator helped construct the diagnosis records. The reason for admission was not recorded. The delivered files were labelled `chd_inpatient` and `hf_inpatient` for each calendar month. We treat those labels as inpatient counts. Age and sex breakdowns were not included. A stroke file was named in correspondence but was not provided, so we do not analyse stroke. ICD code lists remain with the outcome co-investigator.

Monthly influenza activity from November 2013 through December 2023 came from Centre for Health Protection Flu Express summaries. Flu Express did not publish a usable monthly series for January–October 2013, so those ten months are missing. We did not fill them with zeros. Influenza is not in Model 1.
...
We fitted Model 1 twelve times: six weather variables × two diagnoses. The six weather variables are mean temperature, mean maximum temperature, mean minimum temperature, hot nights, very hot days, and cold days. We used a negative-binomial model because monthly counts varied more than a Poisson model allows. A quasi-Poisson model is a sensitivity.

We report Newey–West standard errors with a lag of 6 months [19], together with model-based, HC1, and Newey–West lag-3 intervals. Monthly hospital counts can be correlated from one month to the next; Newey–West intervals allow for that.

Model 2 adds monthly total rainfall and monthly mean relative humidity to Model 1. Goggins and Chan included humidity among meteorological predictors of daily heart-failure admissions in Hong Kong [21]. Table 2 reports Model 1.

Model 3 uses the same twelve fits as Model 1, but replaces official hot-night, very-hot-day, and cold-day counts with days warmer than the historical same-date mean and days cooler than that mean, also scaled per five days. Table 2 reports Model 1.

### Sensitivity analysis

Official day-count estimates from Model 1 can also be read per 3 days or per 1 day without fitting a new model. If the count ratio per five days is *R*, the count ratio per three days is *R*^(3/5) and the count ratio per one day is *R*^(1/5). Those are the same coefficient on a different scale. Cold-day models restricted to November–March are a labelled sensitivity. Other checks were: a smoother time curve (3, 6, or 8 degrees of freedom); year indicators instead of the time curve; dropping the first 12 or 24 months; using only January 2013–December 2019 (84 months); adding COVID-period indicators; lagging the weather variable by 0, 1, or 2 months; and dropping the month with the largest Cook’s distance. COVID periods were: through January 2020; February 2020–December 2021; January–April 2022; May–December 2022; and from January 2023. We inspected residual correlation at lags 6 and 12. We computed Benjamini–Hochberg *q*-values across the twelve Model 1 fits [20]. Models that added pollution or influenza are extra checks. They are not replacements for Model 1.

We did not apply a published method for recovering daily weather effects from monthly totals [15,16] to the hospital counts. A full daily lag model cannot be identified from 132 monthly totals.

Analyses were run in R 4.3.3. Negative-binomial models used `MASS::glm.nb`. Newey–West and HC1 standard errors used `sandwich`. The time curve used `splines::ns`.
...
Analysis code is at https://github.com/bobshenruililin/Laidlaw-Heat-Project. Monthly hospital counts are not posted. What may be shared later depends on the institutional review application and the outcome co-investigator.
