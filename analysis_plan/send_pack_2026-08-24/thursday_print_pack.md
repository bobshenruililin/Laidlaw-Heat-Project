# Thursday 27 August — print pack

**25 August overlay:** use [`../send_pack_2026-08-25/print_checklist.md`](../send_pack_2026-08-25/print_checklist.md) and [`../send_pack_2026-08-25/thursday_meeting_briefing.md`](../send_pack_2026-08-25/thursday_meeting_briefing.md). Hogan review is not required for this signature.

Bring paper. Do not use the meeting to reopen the essay unless he requires a correction. Do not bring the live manuscript as a substitute report.

## Print

1. Form 2a with **Bob’s fields complete** and the supervisor block blank.
2. `supervisor_one_pager.html` printed to one A4 sheet (matches the 24 August conclusion).
3. Title page plus conclusion of the **24 August** `ShenRuililin_Laidlaw_Stage3Report.pdf`, or the full essay if a stapler is easy.
4. Optional: reduced A4 proof of the poster. The official file remains A0.

## First ten minutes (the only programme ask)

- This is the HKU Laidlaw research-report form, due 31 August.
- Please use the 24 August PDF, not the 22 August file.
- Rating / comments / signature. Optional paste is on the one-pager.
- Poster is 15 September; no signature today.
- Journal review with Hogan is a separate file.

## After the form (if there is time)

Weather lock, live-manuscript paste, stroke, person-time, and Gate 3 stay off the form. They can be talked about. They must not delay the signature.

## If he wants to take it away

Leave the printed form and one-pager. Ask for return by Friday 28 August. Send a same-day thank-you with no new attachments.
