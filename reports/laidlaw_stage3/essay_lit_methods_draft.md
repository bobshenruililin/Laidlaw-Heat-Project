...
### 4.7 Jasmine, Roro’s baseline, and plural hot/cold months

“Jasmine’s paper” is now locked as Jingwen Liu et al. (2020): cause-specific mortality attributable to cold and hot ambient temperatures in Hong Kong, 2006–2016 (*Sustainable Cities and Society* 57:102131; DOI `10.1016/j.scs.2020.102131`). Using a DLNM within quasi-Poisson regression, that paper reported a reversed J-shaped temperature–mortality curve and a much larger attributable fraction for cold (4.72%) than for heat (0.16%), with moderate non-optimum weather (AF 4.25%) outweighing extremes (0.63%). That cold ≫ heat burden is qualitatively compatible with Professor Bishai’s recollection of fewer “null” results on the colder side—but attributable fractions are not p-value counts, and the two must not be equated.

A second team baseline is Roro’s excess heat-mortality manuscript (Zhenyuan Liu, Chao Ren, Jingwen Liu, Kawasaki Yurika, and David Bishai; medRxiv DOI `10.64898/2026.03.05.26347683`; 2014–2023). It transports local RRs from Jasmine and from Wang/Ren (2019) into absolute excess deaths under four heatwave definitions (`HWD_Tavg` at mean temperature 30.60°C with Jasmine’s 20-day lag window; `HWD_Tmax`; `HWD_Tmin`; `HWD_Tcombined` / 2D3N), yielding 1,455–3,238 modelled excess deaths depending on definition. Jingwen Liu (Jasmine) is a coauthor; Chao Ren and Professor Bishai close the loop. The Mac file `revised manuscript_clean.pdf` has not yet been ingested here—the deep-read uses medRxiv v1 pending that revision. Our monthly stroke panel is complementary to both papers, not a replacement: morbidity rather than mortality; paired hot *and* cold months; later-period stroke aggregates with Roro’s true-event-month rule.

A “hot month” or “cold month” is not a natural binary. The project catalogue therefore retains fifty hot and forty-eight cold definitions (pathways H01–H12 / C01–C12; see `analysis_plan/hot_cold_month_catalogue.md`), including HKO extremes, Wang/Ren spells, Hogan’s Atmos Res count→upper-tail recipe (HM23), and a hypothesis-motivated ≤10°C cold sensitivity (CM05). Starters freeze with Hogan before outcomes. Jasmine’s definitions, once fully transcribed from the paper/supplement, will be extended definition-harmonised through December 2023; if the estimand remains daily mortality while ours is monthly stroke, that work is **definition transport**, not an exact replication. South China heat resistance around ~10°C remains a discussion hypothesis only. No stroke coefficients are yet available.
...
The near-term team asks are small and concrete: Hogan—lock heatwave / hot-month / cold-month starters together (including Roro’s four mortality defs as named siblings); Roro—confirm first-GOPC→true-stroke timing, the aggregate transfer path, and whether `revised manuscript_clean.pdf` differs from medRxiv v1; Bob—finish the Jasmine full-text source manifest once the SCS PDF is in hand. Going far requires going together.
...
Liu J, Hansen A, Varghese B, et al. (2020). Cause-specific mortality attributable to cold and hot ambient temperatures in Hong Kong, 2006–2016. *Sustainable Cities and Society* 57:102131. DOI 10.1016/j.scs.2020.102131. (**Jasmine**)  
Liu Z, Ren C, Liu J, Kawasaki Y, Bishai DM. (2026). Modelling the excess mortality associated with heat waves in Hong Kong: 2014–2023. *medRxiv*. DOI 10.64898/2026.03.05.26347683. (**Roro baseline**)  
