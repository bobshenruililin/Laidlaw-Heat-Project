...
\begin{frame}[plain]
  ...
  \begin{center}
  \scriptsize
  {\color{LaidlawTeal}\textbf{\footnotesize READY:}} 132-month HKO exposure file
  \hspace{0.9em}
  {\color{LaidlawRust}\textbf{\footnotesize PENDING:}} HA/HPC access and final schema
  \hspace{0.9em}
  {\color{LaidlawNavy}\textbf{\footnotesize TODAY:}} analysis unit, lag plan, next deliverable
  \end{center}
\end{frame}
...
\begin{frame}{What is ready for the first merge}
  ...
  \begin{tikzpicture}
    \node[
      ...
      \textbf{Current boundary:} exposure file and merge pipeline are ready;
      real HA associations await HPC access and row-definition confirmation.
    };
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Monthly temperature at HKO Headquarters, 2013--2023}
  ...
  \vspace{-0.35em}
  \begin{center}
  \scriptsize Environmental exposure data only; no admission outcomes shown.
  \end{center}
\end{frame}
...
\begin{frame}{Table 1 is partly complete}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \footnotesize
      \begin{itemize}
        \item Patients and qualifying episodes
        \item Age and sex
        \item Medication fields: diuretics, beta blockers, metformin, SGLT2 inhibitors
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{tikzpicture}[
        ...
        \node[box, below=of c] (d) {Merge HKO exposure + C\&SD person-time};
    ...
    \begin{column}{0.40\textwidth}
      ...
      \begin{tikzpicture}
        \node[
          ...
          {\color{LaidlawTeal}\textbf{If non-event follow-up exists:}}\\[0.15em]
...
\begin{frame}{Temperature variables are ready; here is the proposed lag plan}
  \begin{columns}[T]
    ...
    \begin{column}{0.54\textwidth}
      ...
      \begin{enumerate}
        ...
        \item Fit temperature specifications separately rather than in one saturated model.
        \item Proposed first sequence:
          \begin{itemize}
            \item same-month exposure as initial specification
            \item lag-1 exposure as sensitivity
            \item current/previous-month average as secondary
          \end{itemize}
        \item Extend weather input to December 2012 so January 2013 has valid lag-1.
      \end{enumerate}

      \vspace{0.35em}
      {\color{LaidlawNavy}\textbf{Decision for today:}} primary temperature metric ---
      mean temperature, Tmin/Tmax, or extreme-day counts.
    \end{column}
  \end{columns}
\end{frame}

%----------------------------------------------------------------------
% Frame 7 --- Confirmations
\begin{frame}{Decisions that unlock the first HA report}
  \footnotesize
  \begin{enumerate}
    ...
    \item \textbf{Analysis population}\\[0.1em]
      ...
      primary temperature metric for the first model.
  ...
  \begin{tikzpicture}
    \node[
      ...
      validated HA data dictionary/QC report, descriptive admission counts,
      age-specific rates, and completed Table~1.\\[0.25em]
...
\begin{frame}{A2. Full HKO validation table (annual extremes)}
  \footnotesize
  Station: HKO Headquarters. Result: all 33 annual extreme-day totals matched
  HKO \textit{The Year's Weather}.
...
\begin{frame}{A4. Pipeline / data flow}
  ...
  \begin{tikzpicture}[
    ...
    box/.style={draw=LaidlawNavy, rounded corners=2pt, align=center, font=\footnotesize,
      inner sep=4pt, text width=2.85cm, fill=LaidlawSoft},
    arr/.style={-{Latex}, thick, LaidlawTeal}
  ]
    ...
    \node[box, below=of desc, text width=4.8cm] (mod) {%
      \centering
      \mbox{Admission-rate/count models}\\
      \mbox{after definitions locked}};
    \draw[arr] (hko) -- (merge);
    \draw[arr] (csd) -- (merge);
    \draw[arr] (ha) -- (merge);
    \draw[arr] (merge) -- (desc);
...
\begin{frame}{A6. Deferred pollution, district, and 2024 extensions}
  \footnotesize
  \begin{itemize}
    \item \textbf{Pollution:} EPD import scripts are ready; the current file is a placeholder
      and will not be used for inference. Near-term priority remains
