...
| A63 | Health-economics layer | Explore-only identification laboratory (`analysis_plan/health_econ/`). Naive RR×events×unit-cost is rejected. No monetisation, no stroke economics, no Gate 3 input, no pathway promotion without a human nod. Monte Carlo counts are `SYNTHETIC_CALIBRATION` on `REAL_PUBLIC_HKO` exposures | Low (scope) | Prevents a costing bolted onto exploratory count ratios | Keep the failed-hypothesis registry; do not paste lab output into the live manuscript | Bob/Bishai |
