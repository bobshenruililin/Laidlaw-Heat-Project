...
9. **Discussion / Conclusion / limitations.** Replace. Flu numeral 1.673 is out of the body (direction only; numeral in the release). CHD hot-night window dependence and spline entanglement are explicit. Conclusion remains comparative, not superlative. **Contribution paragraph (23 August, Fable-bounded):** paste the aggregation-identifiability and calibrated-refusal paragraph from `Heat_CVD_Manuscript_live_update.md`. It claims the framework *for this extract*, cites [15,16], treats daily recovery as a failed calibration not an impossibility, and splits AMI/stroke/incidence refusals onto missing fields. Do not add coefficients. Do not paste F1.2, Gate 3, or health-econ. Weather paragraph still untouched.
