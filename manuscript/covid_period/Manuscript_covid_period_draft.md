...
The dependent variable is the monthly count of first hospitalisations after a first diagnosis of coronary heart disease (CHD), and separately of heart failure (HF), among people with type 2 diabetes and/or hypertension in Hong Kong. The cohort is people diagnosed with type 2 diabetes and/or hypertension during 2013–2023. For each person, the event is the first recorded hospitalisation after that person’s first CHD diagnosis or first HF diagnosis. Admission cause was not recorded. The monthly transfer does not specify a look-back or washout, so “first” may be first in the 2013–2023 window rather than lifetime first. It also does not specify whether the index stay is the diagnosis episode or a later any-cause stay, nor the ICD list or whether the count includes day-procedure or emergency-department records. Age and sex strata were not included. Laboratory measurements were not in the transfer and are not analysed.
...
We report 95% confidence intervals in four ways: the model’s own standard errors, HC1, Newey–West with a lag of 3 months, and Newey–West with a lag of 6 months [17]. Displayed official-day intervals in the text and in Table 2 use the lag-6 construction as a display convention. That convention is not a reason to treat exclusion of 1 as a finding. The four-construction ladder is always shown for CHD hot nights and HF cold days (Table 5). No interval is chosen because it excludes 1. Monthly hospital counts can be correlated from one month to the next. Newey–West intervals allow for that. A *q*-value is a *p*-value adjusted for testing the twelve Model 1 fits together [18]. Multiplicity-adjusted *q*-values were computed from the twelve full-window Newey–West lag-6 Wald *p* values only, not from model-based *p*. Nested-window, COVID-phase, spline-df, influence-month, and joint-model fits are outside that family. Nested-window intervals have no multiplicity control and are not promoted as supporting evidence.

The nested specification repeats equation (1) on January 2013–December 2019 (84 months). Those 84 months are contained in the 132. This is an overlapping-sample refit, not a nested generalised linear model: calendar-month factors and the 4-df natural cubic spline are re-estimated on the subset, and spline knots follow the subset’s time-index quantiles. The two fits are therefore not independent periods, and their difference is not a fitted pre/post effect. No interaction between exposure and period was fitted, and no 48-month post-2020 window is reported as a Model 1 table. COVID-period indicators on the full sample (through January 2020; February 2020–December 2021; January–April 2022; May–December 2022; and from January 2023) are five intercept levels added to month factors and the 4-df spline. They are a full-sample adjustment, not a third window and not an exposure × period interaction. They do not replace Model 1.
