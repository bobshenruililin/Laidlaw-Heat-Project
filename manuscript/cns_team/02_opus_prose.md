# CNS-register drop-ins for the live collaborative manuscript

**Role:** publication-register writing pass (Opus) on `manuscript/live_collaborative/Heat_CVD_Manuscript_live_update.md`.
**Status:** paste-ready section drop-ins. The live file is not edited here; the parent session merges.
**Provenance rule applied:** every numeral below was re-checked against `HA_APPROVED_AGGREGATE` or `REAL` tables in `outputs/`, or against a paper listed in `manuscript/live_collaborative/claim_ledger.md`. Audit trail in `02_opus_cns_checklist.md`.
**Rails observed:** no protected primary claim, no team freeze declared, all twelve core *q* > 0.19 retained, Hogan's weather paragraph verbatim, Author 2–4 placeholders retained, references numbered [1]–[21] as in the live draft.
**Architecture note:** `manuscript/cns_team/01_fable_architecture.md` did not exist when this pass began, so the section order follows the live draft and the mandatory items of `knowledge/2026-08-13_fable_live_doc_critique.md`.

Editorial markers are HTML comments so that they disappear from rendered prose. Two are load-bearing: `HOGAN_WEATHER_VERBATIM` (do not edit the enclosed paragraph) and `OUTCOME_CO-I_TO_CONFIRM` (the outcome co-investigator replaces the enclosed paragraph where the delivered field definitions differ).

---

## TITLE

**Recommended (unchanged from the live draft):**
...
**Optional variant, if the team wants the design signalled in the title:**

Thermal extremes and first hospitalisation after coronary heart disease or heart failure diagnosis in Hong Kong: what 132 months of aggregate counts identify, 2013–2023
...
- Recommends keeping the live title unchanged; the alternative is offered as a variant rather than a replacement, because the live title already names the outcome construction and the window.
- The variant states the design contribution without claiming originality or naming a framework.
- Author block, footnote, and keywords are reproduced without change.
...
*(250 words in the four labelled sections, counted excluding the section labels.)*

**DIFF vs live draft**

- Cut to 250 words from 296. The exposure list is compressed to "three continuous temperature measures" plus the three official counts, which also removes the ambiguity of naming a monthly "maximum temperature" in an abstract; the exact definitions are in Methods.
- Removed the sentence on the simulated daily-recovery method, which Methods and Discussion both carry, and the two residual-autocorrelation values, which Results and the Supplement carry.
- Added the two window sensitivities that decide how each association should be read: the CHD hot-night pre-2020 estimate (1.011, 0.991–1.032) and the HF cold-day pre-2020 estimate (1.113, 1.053–1.176). The live Abstract gave the *q*-values and the standard-error ladder but not the window dependence, which is the asymmetry the 13 August critique flagged.
- Background is two sentences instead of three, and states the gap rather than restating the exposure trend twice.
- The Newey–West lag-3 unrounded bounds stay in Results and Discussion, where there is room to state them exactly, rather than as "on the unrounded scale" in the Abstract.
...
Air pollution also changed during the present study window. At general monitoring stations, mean nitrogen dioxide declined from 53.7 µg m^−3^ in 2013 to 32.1 µg m^−3^ in 2023, and fine particulate matter (PM2.5) from 30.8 to 14.6 µg m^−3^ [7]. Ozone rose over the same years, from 42.6 to 58.3 µg m^−3^ [7]. Temperature and pollutants are not interchangeable covariates. Ozone is coupled with hot, sunny conditions, and may confound the thermal association or lie on its pathway.
...
This study estimates associations between specified monthly thermal-exposure contrasts and monthly counts of first hospitalisation after first CHD diagnosis, and after first HF diagnosis, among people with type 2 diabetes and/or hypertension in Hong Kong from January 2013 through December 2023. Heat and cold exposures are carried in parallel. The analysis is organised around what monthly aggregate counts can identify. The complete twelve-contrast panel is reported rather than its largest members. Uncertainty is displayed as a ladder of standard-error constructions rather than as a single interval. The sources of cold-day variation and of the decline in first events are shown directly. A method for recovering daily exposure coefficients from monthly sums is reported as a refusal rather than as an estimate.

**DIFF vs live draft**

- Sentence one now cites [9] alone. Reference [10] is Bhaskaran's time-series methods tutorial and supports the Statistical analysis section, not a substantive claim about humid heat.
- The cold-day sentence carries [6] alongside [2,4], because the 2023 value comes from the Observatory open-data series rather than from the 2013 or 2021 yearbook.
- Removed "therefore" from the hot-night encoding sentence and replaced it with "by construction". The previous connective implied that Guo et al. had tested monthly counts; they did not [17].
- The final paragraph now states the contribution as four concrete reporting decisions instead of one sentence about discordance. This is the aggregation-aware identification claim, and it is the only claim of contribution in the paper.
- No change to the Goggins and Chan [21] or Guo et al. [17] bounding sentences; they were already precise about design, period, and lag window.
- Outstanding for the weather co-investigator: references [3] (2017 yearbook) and [5] (2024 yearbook) remain uncited in the body. References 1–8 are his block, so they are flagged rather than deleted here.

---

## METHODS_EXCEPT_HOGAN_WEATHER
...
<!-- OUTCOME_CO-I_TO_CONFIRM: the outcome co-investigator holds the field-level ICD lists, the executable first-event timing rule, and the inpatient versus day-and-emergency semantics. Replace this paragraph where the delivered definitions differ from it. -->

Governed monthly aggregates were constructed and released by the outcome co-investigator. The cohort comprised people diagnosed with type 2 diabetes and/or hypertension during 2013–2023. For each outcome, the event was the first recorded hospitalisation after the patient's first diagnosis record for CHD or for HF. Diagnosis-record construction was assisted by a clinical collaborator. Admission cause was not recorded. Delivered columns were labelled `chd_inpatient` and `hf_inpatient` at territory × calendar-month grain. Inpatient-only semantics are inferred from those labels and remain subject to confirmation. Age–sex strata were not included. A stroke series was named in correspondence but was not attached, and no stroke analysis is reported.

### Weather and pollutants data

<!-- HOGAN_WEATHER_VERBATIM — begins. Do not edit, reword, or re-punctuate the following paragraph; it is the weather co-investigator's Methods text as it stands in the live file. -->
...
Core intervals used Newey–West standard errors with lag 6 [19], reported together with model-based, HC1, and Newey–West lag-3 intervals. Lag 6 was the core reporting choice because residual serial dependence is expected in monthly counts and is material for CHD. Robust intervals are not automatically wider than model-based intervals, and the four-construction ladder is reported for that reason. No interval was selected because it excluded 1.
...
- The weather paragraph is enclosed in `HOGAN_WEATHER_VERBATIM` markers and is byte-identical to the live file, including "Meteorological data was obtained from the HKO."
- Health data: the instruction that the outcome co-investigator should replace the paragraph moved from the body into the `OUTCOME_CO-I_TO_CONFIRM` comment. The scientific content is unchanged; the body no longer addresses a collaborator in the third person.
- Statistical analysis: "twelve contrasts" is now defined by the sentence that lists the six exposures for two outcomes, rather than asserted in parentheses.
- Weather processing, denominators, influenza, daily-recovery, software, and governance paragraphs were split into shorter sentences with one claim each. No procedure, threshold, or number changed.
- "Frozen calibration gates" became "frozen calibration criteria" to keep internal gate vocabulary out of the body.
- Study design keeps the core-panel definition sentence required by the 13 August critique.
...
All twelve *q*-values exceeded 0.19, with a minimum of 0.192. No contrast meets a multiplicity-protected confirmatory threshold in this exploratory panel. The HF mean-minimum-temperature interval included 1 at the fourth decimal place, with an unrounded upper bound of 1.00005.
...
Pearson residual autocorrelation at lag 1 was 0.508 in the CHD hot-night model and 0.146 in the HF cold-day model (see Supplement). Ljung–Box tests at lag 6 gave *p* < 10^−7^ for every CHD core model and *p* > 0.3 for every HF core model. Serial dependence is part of the inferential problem for CHD, which is why the four-construction ladder is shown rather than a single interval.
...
**Figure 3. Core count ratios across trend, window, and COVID-phase specifications (Newey–West lag-6 intervals).** All twelve core contrasts are shown across nine specifications. The pre-2020 CHD hot-night interval includes 1, while the pre-2020 HF cold-day interval strengthens. Provenance: HA_APPROVED_AGGREGATE estimates from the trend and depletion sensitivity table.

![Figure 3](../../outputs/release_chd_hf/figures/figure4_trend_depletion_sensitivity.png)
...
- Figure 3 is now the trend and depletion sensitivity display from the validated release, and the residual autocorrelation figure moves to the Supplement. The autocorrelation figure showed two numbers that already appeared in the Abstract, the Results text, and its own caption; the replacement answers an identification question that no table in the paper answers.
- The autocorrelation values are now one Results sentence with a Supplement pointer, and the Abstract no longer repeats them.
- Ljung–Box lag-6 reporting changed from "*p* < 10^−8^" to "*p* < 10^−7^". The largest CHD core lag-6 *p*-value is 1.17 × 10^−8^, which is above 10^−8^; the previous threshold was false for the hot-night model by a hair, and a referee who reads the Supplement would find it.
- Results gained one sentence noting that first-event decline and hot-night increase are both strong trends absorbed by the same spline. This is the identification cost that Figure 3 makes visible.
- Diagnostics and sensitivities were split into two subsections, so that the joint-model collinearity diagnostics are not read as robustness evidence.
- Exposure context: "the highest annual number of hot nights in the Observatory record through the study window" became "the highest annual number of hot nights in the Observatory record at that time [4]", which is what the 2021 yearbook supports. Reference [6] is attached to the 2023 exposure counts.
- Table 2's *q* sentence now states the minimum (0.192) once, so that the Abstract and Results agree on the same wording.
- "Frozen calibration gates" became "worst-cell criteria" in the daily-recovery paragraph. The four failing values are unchanged.
...
Pollution and influenza are scientifically motivated in this setting [1,18]. Archive models in which they enter are not adjusted versions of the core panel. On the 121 months with influenza data, an archive model associated higher influenza activity with higher CHD counts; that estimate is reported in the accompanying release, and it does not adjust Table 2. Confounding by infection, ozone, or nitrogen dioxide is therefore unresolved. Absent cohort person-time, even a stable count ratio remains a count ratio [14].
...
- Paragraph 2 gains the pre-2020 strengthening of the HF cold-day estimate, so that both leading contrasts are described with the same window evidence.
- Paragraph 3 gains the depletion-and-exposure trend point: the CHD hot-night estimate is confined to specifications including 2020–2023, and the same spline controls both the outcome decline and the hot-night rise. The 13 August critique identified this as the fact that makes the encoding question urgent for the weather co-investigator.
- The archive influenza numeral is removed and replaced by direction plus a pointer to the release. The coefficient is the largest confidence-bounded number anywhere in the paper, it is not from the analysis of record, and skimmed numerals travel further than the disclaimers wrapped around them.
- Deleted "and sleep, blood pressure, and personal exposure were not measured" from the Guo paragraph. Naming unmeasured mediators invites a mechanism the design cannot support; the Limitations already state that the design is ecological and monthly.
- Added one paragraph that states the contribution in plain terms before the closing bar-setting sentence. It claims reporting practice, not discovery.
- The ladder-direction sentence is retained from the 13 August critique in place of the earlier claim that model-based intervals are too narrow for the CHD series, which the paper's own Table 3 contradicts.
...
- Split the CHD caveat into its own sentence, and added the window dependence to it, matching Results and Discussion.
- Retains the comparative wording ("more closely associated ... than with the other thermal encodings examined") rather than a superlative rank across an unprotected panel.
- No numerals in the Conclusion, so nothing here can drift from Tables 2 and 3.
...
**Limitations.** Admission cause was not recorded, so an event is a first hospitalisation after a first diagnosis and not a cardiac-caused admission. Inpatient-only semantics are inferred from the delivered column labels and are not yet confirmed. Monthly counts of cohort members still at risk of a first event were unavailable, so the estimates are count ratios rather than incidence-rate ratios [14]. The design is ecological and monthly, so individual-level and daily-triggering interpretations are not identified [14]. Age, sex, and disease-subtype strata were not delivered. Residual serial correlation remains material for CHD, with lag-1 Pearson autocorrelation of 0.508 in the hot-night model. Official cold days are concentrated in December–February, so the HF cold-day estimate rests on differences between winters. The CHD hot-night estimate is not stable across analysis windows, and is compatible with 1 before 2020. Official hot-night counts are not hourly nighttime excess heat [17]. All exposures were measured at a single Observatory station and applied territory-wide, and within-territory exposure variation was not modelled. Reference rules for monthly hot-tail and cold-tail indicators are not yet fixed, and no such indicator was used as a confirmatory exposure. Confounding by pollution, humidity, and influenza is unresolved in the core panel. A stroke series was named in correspondence but was not delivered, and no stroke result is reported. External dissemination of the governed aggregates requires confirmation from the outcome and supervising investigators.

**DIFF vs live draft**

- Strengths: the identification-displays sentence now includes window dependence, which is what the new Figure 3 shows.
- Limitations: added that the CHD hot-night estimate is not stable across analysis windows and is compatible with 1 before 2020. The live version stated the serial-correlation limit but not the window limit.
- Single-station exposure limitation retained from the 13 August critique.
- Otherwise unchanged in content; sentences split so that each carries one limitation.

---

## Notes for the parent session

1. **Not written into the prose, deliberately.** The number of months carrying any official cold day is 29 of 132 (`outputs/live_identification/cold_days_by_month_year.csv`, REAL). The 13 August critique estimated "roughly 33–37 months" as an EXPLORE-ONLY item; the verified count is lower. It is outside the allowed number set for this pass, so it is recorded here rather than in the manuscript. If the team wants the effective-information point in the paper, this is the number to use, and it strengthens the between-winters identification argument.
2. **Reference hygiene left open.** References [3] and [5] remain uncited in the body. References 1–8 are the weather co-investigator's block, so pruning or attaching them is his call.
3. **Supplement consequence of the figure swap.** `figures/live_identification/figure_C_residual_acf.png` is now a Supplement figure and needs a Supplement caption; the two values it displays are already in Results.
4. **Unchanged sections.** Declaration on the use of AI, competing interests, Acknowledgements, data and code availability, and References [1]–[21] are not restated here, and no change to them is proposed.
